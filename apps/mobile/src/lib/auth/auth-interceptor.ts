...
class AuthInterceptor {
      ...
      async (config: AxiosRequestConfig) => {
        try {
          ...
          config.headers = {
            ...
            'User-Agent': this.getUserAgent(deviceInfo),
          };
   ...
   */
  createAuthenticatedInstance(baseURL?: string): typeof axios {
    const instance = axios.create({
      baseURL: baseURL || process.env.EXPO_PUBLIC_API_URL || 'http://localhost:3000/api',
   ...
   */
  private applyInterceptorsToInstance(instance: typeof axios): void {
    // Request interceptor
   ...
   */
  private async handleResponseError(error: AxiosError, instance: typeof axios): Promise<any> {
    const originalRequest = error.config as AxiosRequestConfig & { _retry?: boolean };
