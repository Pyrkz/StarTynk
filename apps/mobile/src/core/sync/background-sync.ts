...
export class BackgroundSyncManager {
  ...
  private async fetchLatestData(): Promise<{ hasNewData: boolean }> {
    try {
      ...
      const updates = await trpcClient.sync.getUpdates.query({
        since,
        entities: ['projects', 'tasks', 'attendance'],
      });
      
      if (updates.data && Object.keys(updates.data).length > 0) {
        // Transform raw data to ServerUpdate format
        const transformedUpdates: Record<string, Array<{
          entityType: string;
          entityId: string;
          data: any;
        ...
        for (const [entityType, entities] of Object.entries(updates.data)) {
          transformedUpdates[entityType] = (entities as any[]).map(entity => ({
            entityType,
            entityId: entity.id,
            data: entity,
        ...
        await dataSync.applyServerUpdates(transformedUpdates);
